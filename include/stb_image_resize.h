...
static float stbir__filter_trapezoid(float x, float scale)
{
    ...
    (void) STBIR__DEBUG_ASSERT(scale <= 1);
...
static void stbir__calculate_coefficients_upsample(stbir__info* stbir_info, stbir_filter filter, float scale, int in_first_pixel, int in_last_pixel, float in_center_of_out, stbir__contributors* contributor, float* coefficient_group)
{
    ...
    (void) STBIR__DEBUG_ASSERT(in_last_pixel - in_first_pixel <= (int)ceil(stbir__filter_info_table[filter].support(1/scale) * 2)); // Taken directly from stbir__get_coefficient_width() which we can't call because we don't know if we're horizontal or vertical.

    contributor->n0 = in_first_pixel;
    contributor->n1 = in_last_pixel;

    (void) STBIR__DEBUG_ASSERT(contributor->n1 >= contributor->n0);
    ...
    (void) STBIR__DEBUG_ASSERT(stbir__filter_info_table[filter].kernel((float)(in_last_pixel + 1) + 0.5f - in_center_of_out, 1/scale) == 0);

    (void) STBIR__DEBUG_ASSERT(total_filter > 0.9);
    (void) STBIR__DEBUG_ASSERT(total_filter < 1.1f); // Make sure it's not way off.
...
static void stbir__calculate_coefficients_downsample(stbir__info* stbir_info, stbir_filter filter, float scale_ratio, int out_first_pixel, int out_last_pixel, float out_center_of_in, stbir__contributors* contributor, float* coefficient_group)
{
    ...
    (void) STBIR__DEBUG_ASSERT(out_last_pixel - out_first_pixel <= (int)ceil(stbir__filter_info_table[filter].support(scale_ratio) * 2)); // Taken directly from stbir__get_coefficient_width() which we can't call because we don't know if we're horizontal or vertical.
    ...
    (void) STBIR__DEBUG_ASSERT(stbir__filter_info_table[filter].kernel((float)(out_last_pixel + 1) + 0.5f - out_center_of_in, scale_ratio) == 0);
...
static void stbir__normalize_downsample_coefficients(stbir__info* stbir_info, stbir__contributors* contributors, float* coefficients, stbir_filter filter, float scale_ratio, float shift, int input_size, int output_size)
{
    ...
    for (i = 0; i < output_size; i++)
    {
        ...
        (void) STBIR__DEBUG_ASSERT(total > 0.9f);
        (void) STBIR__DEBUG_ASSERT(total < 1.1f);
...
static float* stbir__add_empty_ring_buffer_entry(stbir__info* stbir_info, int n)
{
    ...
    else
    {
        ...
        (void) STBIR__DEBUG_ASSERT(ring_buffer_index != stbir_info->ring_buffer_begin_index);
    }
...
static void stbir__resample_horizontal_upsample(stbir__info* stbir_info, int n, float* output_buffer)
{
    ...
    for (x = 0; x < output_w; x++)
    {
        ...
        (void) STBIR__DEBUG_ASSERT(n1 >= n0);
        (void) STBIR__DEBUG_ASSERT(n0 >= -stbir_info->horizontal_filter_pixel_margin);
        (void) STBIR__DEBUG_ASSERT(n1 >= -stbir_info->horizontal_filter_pixel_margin);
        (void) STBIR__DEBUG_ASSERT(n0 < stbir_info->input_w + stbir_info->horizontal_filter_pixel_margin);
        (void) STBIR__DEBUG_ASSERT(n1 < stbir_info->input_w + stbir_info->horizontal_filter_pixel_margin);
        ...
        switch (channels) {
            case 1:
                for (k = n0; k <= n1; k++)
                {
                    ...
                    (void) STBIR__DEBUG_ASSERT(coefficient != 0);
...
static void stbir__resample_horizontal_downsample(stbir__info* stbir_info, int n, float* output_buffer)
{
    ...
    (void) STBIR__DEBUG_ASSERT(!stbir__use_width_upsampling(stbir_info));
...
static void stbir__resample_vertical_upsample(stbir__info* stbir_info, int n, int in_first_scanline, int in_last_scanline, float in_center_of_out)
{
    ...
    (void) STBIR__DEBUG_ASSERT(stbir__use_height_upsampling(stbir_info));
...
static void stbir__buffer_loop_upsample(stbir__info* stbir_info)
{
    ...
    for (y = 0; y < stbir_info->output_h; y++)
    {
        ...
        (void) STBIR__DEBUG_ASSERT(in_last_scanline - in_first_scanline <= stbir_info->vertical_filter_pixel_width);
...
static void stbir__buffer_loop_downsample(stbir__info* stbir_info)
{
    ...
    for (y = -pixel_margin; y < max_y; y++)
    {
        ...
        (void) STBIR__DEBUG_ASSERT(out_last_scanline - out_first_scanline <= stbir_info->vertical_filter_pixel_width);
...
#define STBIR__NEXT_MEMPTR(current, newtype) (newtype*)(((unsigned char*)current) + current##_size)
    ...
    if (stbir__use_height_upsampling(info))
    {
        ...
        (void) STBIR__DEBUG_ASSERT((size_t)STBIR__NEXT_MEMPTR(info->encode_buffer, unsigned char) == (size_t)tempmem + tempmem_size_in_bytes);
    }
    else
    {
        ...
        (void) STBIR__DEBUG_ASSERT((size_t)STBIR__NEXT_MEMPTR(info->ring_buffer, unsigned char) == (size_t)tempmem + tempmem_size_in_bytes);
    }
...
#ifdef STBIR_DEBUG_OVERWRITE_TEST
    (void) STBIR__DEBUG_ASSERT(memcmp(overwrite_output_before_pre, &((unsigned char*)output_data)[-OVERWRITE_ARRAY_SIZE], OVERWRITE_ARRAY_SIZE) == 0);
    (void) STBIR__DEBUG_ASSERT(memcmp(overwrite_output_after_pre, &((unsigned char*)output_data)[begin_forbidden], OVERWRITE_ARRAY_SIZE) == 0);
    (void) STBIR__DEBUG_ASSERT(memcmp(overwrite_tempmem_before_pre, &((unsigned char*)tempmem)[-OVERWRITE_ARRAY_SIZE], OVERWRITE_ARRAY_SIZE) == 0);
    (void) STBIR__DEBUG_ASSERT(memcmp(overwrite_tempmem_after_pre, &((unsigned char*)tempmem)[tempmem_size_in_bytes], OVERWRITE_ARRAY_SIZE) == 0);
#endif
